...
def _get_matches(text_lower: str, terms: set[str]) -> list[str]:
    """Return the actual terms that matched in the text."""
    return [term for term in terms if term in text_lower]


def _get_regex_matches(text_lower: str, patterns: list[str]) -> list[str]:
    """Return the actual substring matches for regex patterns."""
    matches = []
    for pattern in patterns:
        found = re.findall(pattern, text_lower)
        for f in found:
            # Handle cases where findall returns tuples due to groups
...
def extract_text_matches(email_body: str) -> dict[str, list[str]]:
    """
    Returns the exact words/phrases that triggered the ML features.
    This is passed up to the UI so it can underline them for the user.
    """
    text_lower = email_body.lower()
    
    return {
        "llm_tropes": _get_matches(text_lower, LLM_TROPE_WORDS) + _get_matches(text_lower, LLM_TROPE_PHRASES),
        "prompt_leakage": _get_regex_matches(text_lower, PROMPT_LEAKAGE_PATTERNS),
        "out_of_band_excuses": _get_regex_matches(text_lower, OUT_OF_BAND_EXCUSE_PATTERNS),
        "urgency": _get_matches(text_lower, URGENCY_WORDS),
        "financial": _get_matches(text_lower, FINANCIAL_ACTION_WORDS),
        "causal_connectives": _get_matches(text_lower, CAUSAL_CONNECTIVES),
        "personal_osint": _get_matches(text_lower, PERSONAL_OSINT_WORDS),
    }
